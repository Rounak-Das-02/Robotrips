...
const PlayButton = () => (
  <button
    ...
    className={tw(
      ...
      -translate-y-1/2 -translate-x-1/2 bg-white rounded-full font-medium group p-4 shadow-xl cursor-not-allowed`,
    )}
    aria-label="play video"
  >
    <Play className={tw(`w-6 h-6 fill-current text-gray-400 group-hover:text-blue-600 flex-shrink-0`)} />
    <span className={tw(`ml-3`)}>Our Robotaxi</span>
  </button>
);
      ...
      <div className={tw(`flex flex-col max-w-4xl mx-auto pt-28`)}>
        <div className={tw(`w-full`)}>
          <div className={tw(`relative shadow-2xl mx-6 lg:mx-0`)}>
            <img className = {tw(`rounded-lg`)} src = "/images/preferences.png" width="100%" height="80%" />
            <PlayButton />
          </div>
        </div>
      </div>
